...
#include "Chippin_In.h"
#include "Never_Fade_Away.h"w
...
void init() {
    ...
    songs[0].data = examplesong_data;
    songs[0].title = "gba bgm!!!";
    songs[0].color = CYAN;

    // TODO 5.3: Set songs[1] and songs[2] struct members

    songs[1].sampleRate = Chippin_In_sampleRate;
    songs[1].length = Chippin_In_length;
    songs[1].data = Chippin_In_data;
    songs[1].title = "Chippin In";
    songs[1].color = BLACK;

    songs[2].sampleRate = Never_Fade_Away_sampleRate;
    songs[2].length = Never_Fade_Away_length;
    songs[2].data = Never_Fade_Away_data;
    songs[2].title = "Never Fade Away";
    songs[2].color = RED;
