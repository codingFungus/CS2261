...
void initBullet() {
    for (int i = 0; i < 25; i++) {
        ...
        bullets[i].yspeed = -3;
        bullets[i].color = (((31) & 31) | ((31) & 31) << 5 | ((31) & 31) << 10);
        bullets[i].active = 0;
        bullets[i].erased = 1;
    }
...
void initEnemy() {
    for (int i = 0; i < 10; i++) {
        ...
        enemies[i].active = 1;
        enemies[i].color = (((0) & 31) | ((31) & 31) << 5 | ((0) & 31) << 10);
        enemies[i].xspeed = 1;
        enemies[i].yspeed = 1;
        enemies[i].erased = 0;
    }
...
void updatePlayer() {
    if (inBorder()) {
        if ((~buttons & (1<<5)) && inSlowZone()) {
            player.x -= player.xspeed/2;
        }
        if ((~buttons & (1<<4)) && inSlowZone()) {
                player.x += player.xspeed/2;
        }
        if ((~buttons & (1<<7)) && inSlowZone()) {
                player.y += player.yspeed/2;
        }
        if ((~buttons & (1<<6)) && inSlowZone()) {
                player.y -= player.yspeed/2;
        }

        else if ((~buttons & (1<<5)) && !inSlowZone()) {
            player.x -= player.xspeed;
        }
        else if ((~buttons & (1<<4)) && !inSlowZone()) {
            player.x += player.xspeed;
        }
        else if ((~buttons & (1<<7)) && !inSlowZone()) {
            player.y += player.yspeed;
        }
        else if ((~buttons & (1<<6)) && !inSlowZone()) {
            player.y -= player.yspeed;
        } else if ((((~buttons & (1<<0)) && !(~oldButtons & (1<<0))))) {
            spawnBullet();
            *(volatile u16*)0x04000068 = (((4) & 15) << 12) | (0 << 11) |
                (((2) & 7) << 8) | (2 << 6);
            *(volatile u16*)0x0400006C = (1<<15) | NOTE_C5;
    ...
    if (e->active) {
        ...
        for (int i = 0; i < 25; i++) {
            if (bullets[i].active && collision(e->x, e->y, e->width, e->height,
                bullets[i].x, bullets[i].y, bullets[i].radius, bullets[i].radius)) {
                    ...
                    *(volatile u16*)0x04000068 = (((4) & 15) << 12) | (0 << 11) |
                    (((2) & 7) << 8) | (2 << 6);
                    *(volatile u16*)0x0400006C = (1<<15) | NOTE_G6;
            }
        }
        if (collision(player.x, player.y, player.width, player.height,
            e->x, e->y, e->width, e->height)) {
                *(volatile u16*)0x04000068 = (((4) & 15) << 12) | (0 << 11) |
                (((2) & 7) << 8) | (2 << 6);
                *(volatile u16*)0x0400006C = (1<<15) | NOTE_C7;
                player.dead = 1;
...
void drawPlayer() {
    drawRectangle(player.oldx, player.oldy, player.width, player.height, (((0) & 31) | ((0) & 31) << 5 | ((0) & 31) << 10));
...
void drawEnemy(Enemy* e) {
    if (e->active) {
        drawRectangle(e->oldx, e->oldy, e->width, e->height, (((0) & 31) | ((0) & 31) << 5 | ((0) & 31) << 10));
